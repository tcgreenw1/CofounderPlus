...
import { Moon, Sun, HelpCircle, Building, Lightbulb, Heart, Target, Users, ChevronLeft } from 'lucide-react';
...
function Questionnaire() {
  ...
  const handleAnswer = (answer: string) => {
    ...
    setTimeout(() => {
      if (answer === 'yes') {
        // Store that this user has an existing business so we can collect their business info after signup
        localStorage.setItem('questionnaire_has_business', 'true');
        // Go directly to signup for existing business owners
        navigate('/auth?from=questionnaire');
      } else {
        // Clear the flag for users who want to start a business
        localStorage.removeItem('questionnaire_has_business');
        // Go to path choice for business planning
  ...
  const helpContent = {
    ...
    description: "We're here to help you decide whether you're ready to scale an existing business or start fresh with a new one.",
    tips: [
      {
        icon: <Building className="w-4 h-4 sm:w-5 sm:h-5" />,
        title: "Choose 'Yes, I own a business' if:",
        description: "You already have a business (even if it's small), generate some revenue, have customers, or have been operating for any amount of time. You'll go straight to creating your account."
      },
      {
        icon: <Lightbulb className="w-4 h-4 sm:w-5 sm:h-5" />,
        title: "Choose 'No, I want to start one' if:",
        description: "You're just getting started, have an idea but haven't launched yet, or want to explore completely new business opportunities. You'll choose from our industry list and then create your account."
      },
      {
        icon: <Target className="w-4 h-4 sm:w-5 sm:h-5" />,
        title: "Both paths lead to success:",
        description: "There's no wrong answer here! We'll create a personalized roadmap based on your current situation, whether you're scaling or starting."
      },
      {
        icon: <Users className="w-4 h-4 sm:w-5 sm:h-5" />,
        title: "You can always pivot:",
        description: "Your journey isn't set in stone. Many successful entrepreneurs have multiple businesses or pivot as they learn and grow."
      }
    ]
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-indigo-50 dark:from-slate-900 dark:via-blue-900 dark:to-indigo-950 starry-background transition-all duration-300">
      {/* Gentle meteors for questionnaire */}
      <div className="shooting-star" style={{ animationDelay: '18s', animationDuration: '5.0s', top: '30%' }}></div>
      <div className="shooting-star" style={{ animationDelay: '58s', animationDuration: '4.7s', top: '75%' }}></div>
          ...
          <div className="space-y-3 sm:space-y-4">
            ...
            >
              <div className="flex items-center space-x-3 sm:space-x-4">
                ...
                <div className="min-w-0 flex-1 text-left">
                  <div className="text-base sm:text-lg">Yes, I own a business</div>
                  <div className="text-sm sm:text-sm opacity-70">Get started right away</div>
                </div>
              </div>
            </motion.button>
            ...
            >
              <div className="flex items-center space-x-3 sm:space-x-4">
                ...
                <div className="min-w-0 flex-1 text-left">
                  <div className="text-base sm:text-lg">No, I want to start one</div>
                  <div className="text-sm sm:text-sm opacity-70">Choose from our industry list</div>
                </div>
              </div>
            </motion.button>
          </div>
              ...
              <CardContent className="p-3 sm:p-4">
                <div className="flex items-start gap-2 sm:gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="font-semibold text-blue-800 dark:text-blue-200 text-sm sm:text-base">You're on the right path!</p>
                    <p className="text-xs sm:text-sm text-blue-700 dark:text-blue-300 leading-relaxed">
                      Every successful entrepreneur started with this exact same question. Trust your instincts and let's build something amazing together.
                    </p>
                  </div>
                </div>
              </CardContent>

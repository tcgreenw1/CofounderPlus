...
interface FastQuizQuestionProps {
...
const QuestionIcon = React.memo(({ dimension, subdimension }: { dimension: string; subdimension: string }) => {
  if (dimension === 'BigFive') {
    ...
    return iconMap[subdimension] || <Brain className="w-8 h-8" />;
  } else if (dimension === 'RIASEC') {
    const iconMap: { [key: string]: any } = {
...
// Memoized option button for better performance
const OptionButton = React.memo(({ 
  ...
  <button
    ...
    className={`w-full p-4 rounded-xl font-medium text-left quiz-button-fast backdrop-blur-lg border flex items-center justify-between transform hover:scale-[1.005] active:scale-[0.995] ${
      isSelected
        ? `bg-gradient-to-r ${dimensionColor} text-white border-transparent shadow-xl`
  ...
  >
    <span className="text-lg">{typeof option === 'object' ? option.label : option}</span>
    {typeof option === 'object' && (
      <span className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${
        isSelected ? 'bg-white/20' : 'bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-300'
      }`}>
        {option.shortLabel}
      </span>
    )}
...
export function FastQuizQuestion({ 
  question, 
  currentIndex, 
  totalQuestions, 
  onAnswer, 
  ...
  canGoBack,
  showBackButton = true
}: FastQuizQuestionProps) {
  const { theme } = useTheme();
  const [localValue, setLocalValue] = useState(selectedValue);
  const [isAnimating, setIsAnimating] = useState(false);

  // Memoized calculations for better performance
  const progress = useMemo(() => ((currentIndex + 1) / totalQuestions) * 100, [currentIndex, totalQuestions]);
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (question.input_type === 'single_choice' && question.options) {
        if (e.key === 'ArrowUp' || e.key === 'ArrowDown') {
          e.preventDefault();
          const currentIndex = question.options.findIndex(opt => opt === localValue);
          let newIndex;
          ...
          handleAnswer(question.options[newIndex]);
        }
        
        if (e.key === 'Enter' && localValue) {
          e.preventDefault();
          handleAnswer(localValue);
        }
      }
    };
  ...
  }, [question, localValue, handleAnswer]);
  ...
  }, [isAnimating, onAnswer]);

  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-indigo-50 dark:from-slate-900 dark:via-blue-900 dark:to-indigo-950 starry-background quiz-container">
      {/* Background elements */}
      ...
      <div className="relative z-10 min-h-screen flex flex-col">
        ...
        <div className="p-6">
          ...
          {/* Progress Bar - Instant CSS transition */}
          <div className="w-full h-3 bg-white/20 dark:bg-gray-800/20 rounded-full backdrop-blur-lg">
            <div
              style={{ width: `${progress}%` }}
              className="h-full bg-gradient-to-r from-purple-600 via-pink-600 to-blue-600 rounded-full quiz-progress-bar"
            />
          </div>
        </div>
        ...
        <div className="flex-1 flex items-center justify-center px-6 py-8">
          <div className="max-w-4xl mx-auto w-full text-center">
            ...
            <div className={`w-20 h-20 bg-gradient-to-r ${dimensionInfo.color} rounded-full flex items-center justify-center mb-8 mx-auto text-white`}>
              <QuestionIcon dimension={question.dimension} subdimension={question.subdimension} />
            </div>
                ...
                <div className="space-y-3">
                  {likertLabels.map((option) => (
                    <OptionButton
                      ...
                      dimensionColor={dimensionInfo.color}
            ...
            {question.input_type === 'likert' && (
              <div className="mt-8 text-sm text-gray-500 dark:text-gray-400">
                ...
                <br />
                <span className="text-xs opacity-75">Tip: Use number keys 1-5 for faster selection</span>
              </div>
            )}
            
            {question.input_type === 'single_choice' && (
              <div className="mt-8 text-sm text-gray-500 dark:text-gray-400">
                <span className="text-xs opacity-75">Tip: Use arrow keys ↑↓ to navigate, Enter to select</span>
              </div>
            )}
          </div>
        </div>
